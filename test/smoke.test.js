...
describe("npm registry check (smoke)", () => {
  it("npm.cmd is callable and returns pluidr version from registry", () => {
    let result
    try {
      if (isWin) {
        result = execSync("npm view pluidr version", {
          stdio: "pipe",
          encoding: "utf-8",
        }).trim()
      } else {
        result = execFileSync("npm", ["view", "pluidr", "version"], {
          stdio: "pipe",
          encoding: "utf-8",
        }).trim()
      }
    ...
    console.log(`  ✓ npm registry returned pluidr@${result}`)
  })
})
